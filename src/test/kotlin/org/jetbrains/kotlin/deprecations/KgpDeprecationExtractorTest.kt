    ...
    fun extracts_all_deprecation_levels() {
        val jar = jarOf(
            "foo/MultiLevel.class" to classBytes(
                ...
                methods = listOf(
                    MethodSpec("a", "()V", DeprecationSpec("WARNING", "w", null)),
                    MethodSpec("b", "()V", DeprecationSpec("ERROR", "e", null)),
                    MethodSpec("c", "()V", DeprecationSpec("HIDDEN", "h", null)),
                )
            )
        )
        ...
        assertEquals(DeprecationLevel.WARNING, symbols["a"]?.level)
        assertEquals(DeprecationLevel.ERROR, symbols["b"]?.level)
        assertEquals(DeprecationLevel.HIDDEN, symbols["c"]?.level)
    }
